...
Service API に対する **L1 / L2 / L3 / D1 / C1** 負荷クライアントです。本番 API / Engine の責務は変更しません。公開件数の正本は [`docs/reference/service-capacity.md`](../../docs/reference/service-capacity.md) です。手順は [`docs/guides/capacity-load-testing.md`](../../docs/guides/capacity-load-testing.md) です。
...
主な引数: `--count`（実行数）、`--concurrency`（同時 HTTP）、`--hw-label`、`--hw-notes`、`--topology`（現行参照は `split-runtime`。既定値は `phase0-single-api`）。D1 は `--restart-service`（既定 `service-api`、git ルートで `docker compose restart`）。D1 / C1 の既定タイムアウトは 300 秒です。記録用: `--worker-replicas` / `--worker-max-concurrency` / `--worker-cancel-concurrency`（サーバ側は変えません）。C1 の settle は `--settle-ms`（既定 3000）。
